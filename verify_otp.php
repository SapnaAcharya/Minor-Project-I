    ...
    if (isset($_POST["email"]) && isset($_POST["otp"])) {
        ...
        if ($user) {
            ...
            if ($user["otp_expiration"] >= $current_time) {
                ...
                // Redirect to password reset page after a delay
                echo '<script>';
                echo 'setTimeout(function () { window.location.href = "reseting_password.php?email=' . $email . '"; }, 2000);'; // Delay of 2 seconds (2000 milliseconds)
                echo '</script>';
                // Redirect the user to the password reset page
                // header("Location: reseting_password.php?email=$email");
            } else {
